...
char *find_hdr_value_end(char *s, const char *e)
{
    ...
    defined(__ARM_ARCH_7A__)) && !defined(__VMS)
...
void http_msg_analyzer(struct http_msg *msg, struct hdr_idx *idx)
{
	...
	http_msg_hdr_val:
		 ...
		 */
#if defined(__x86_64__) && !defined(__VMS)
...
#ifdef __VMS
	const char *ptr = b_ptr((struct buffer *) buf, msg->next);
#else
...
#ifdef __VMS
		const char *start = b_ptr((struct buffer *) buf, msg->next + msg->sol);
#else
...
static inline int http_skip_chunk_crlf(struct http_msg *msg)
{
	...
	bytes = 1;
#ifdef __VMS
	ptr = b_ptr((struct buffer *) buf, msg->next);
#else
...
#ifdef __VMS
		case ACT_ACTION_TRK_SC0:
		case ACT_ACTION_TRK_SCMAX:
#else
...
#ifdef __VMS
	 FLT_STRM_DATA_CB_IMPL_2(s, chn, flt_http_data(s, msg),
...
#ifdef __VMS
        FLT_STRM_DATA_CB_IMPL_2(s, chn, flt_http_forward_data(s, msg, msg->next),
...
#ifdef __VMS
        FLT_STRM_DATA_CB_IMPL_3(s, chn, flt_http_end(s, msg),
                         /* default_ret */ 1,
                         /* on_error    */ goto error,
	...
	switch (msg->msg_state) {
		...
		case HTTP_MSG_TRAILERS:
			...
			if (ret < 0)
				goto chunk_parsing_error;
#ifdef __VMS
			FLT_STRM_DATA_CB_IMPL_2(s, chn, flt_http_chunk_trailers(s, msg),
					 /* default_ret */ 1,
					 /* on_error    */ goto error);
#else
