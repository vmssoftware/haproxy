...
#ifdef __VMS
static int accept_timeout = -1;

#ifndef ACCEPT_TIMEOUT_DEFAULT
...
#ifdef __VMS
	if (accept_timeout < 0) {
		char *tmp = getenv("HAPROXY$ACCEPT_TIMEOUT");
	    	if (tmp) {
			accept_timeout = abs(atoi(tmp));
		} else {
	...
	while (max_accept--) {
		 ...
		 */
   		if (unlikely(accept4_broken ||
			...
			{
				struct pollfd fds[1];
				ioctl(cfd, FIONBIO, (int *) &one);
				fds[0].fd = cfd;
				fds[0].events = POLLIN;
				poll(fds, 1, accept_timeout);
			}
#else
