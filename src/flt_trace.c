...
#ifdef __VMS
// BRC TBD - these need to be made more in line with the macros...
//
static void TRACE(void *conf, char *fmt, ...)
{
        char tmp[32];
        time_t t;
        va_list args;
...
static void STRM_TRACE(void *conf, void *strm, char *fmt, ...)
{
        char tmp[32];
        time_t t;
        va_list args;
