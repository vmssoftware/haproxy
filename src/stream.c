...
#define FLT_ANALYZE1(strm, chn, fun, list, back, flag, arg)			\
	{									\
		if ((list) & (flag)) {						\
			if (HAS_FILTERS(strm)) {			        \
				if (!flt_pre_analyze((strm), (chn), (flag)))    \
					break;				        \
				if (!fun((strm), (chn), (flag), arg))		\
					break;			    		\
				if (!flt_post_analyze((strm), (chn), (flag)))	\
					break;					\
			}							\
...
#ifdef __VMS
				FLT_ANALYZE1(s, req, http_process_req_common,    ana_list, ana_back, AN_REQ_HTTP_PROCESS_FE, sess->fe);
#else
...
#ifdef __VMS
				FLT_ANALYZE1(s, req, http_process_req_common,    ana_list, ana_back, AN_REQ_HTTP_PROCESS_BE, s->be);
#else
...
#ifdef __VMS
				FLT_ANALYZE1(s, res, http_process_res_common,    ana_list, ana_back, AN_RES_HTTP_PROCESS_BE, s->be);
#else
