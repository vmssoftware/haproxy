...
#else
#ifdef __VMS
static inline void SPOE_PRINTF(FILE *x, ...) {}
#else
...
#if defined(DEBUG_SPOE) || defined(DEBUG_FULL) || defined(__VMS)
...
#ifdef __VMS
		ctx = LIST_NEXT(&agent->applet_wq, __typeof__(ctx), applet_wait);
#else
...
acquire_spoe_appctx(struct spoe_context *ctx, int dir)
{
	...
	if (!LIST_ISEMPTY(&agent->cache)) {
#ifdef __VMS
		appctx = LIST_NEXT(&agent->cache, __typeof__(appctx), ctx.spoe.list);
#else
