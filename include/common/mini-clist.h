...
#define list_for_each_entry(item, list_head, member)                      \
	for (item = LIST_ELEM((list_head)->n, __typeof__(item), member);     \
	     &item->member != (list_head);                                \
	     item = LIST_ELEM(item->member.n, __typeof__(item), member))
#else
#define list_for_each_entry(item, list_head, member)                      \
...
#define list_for_each_entry_safe(item, back, list_head, member)           \
	for (item = LIST_ELEM((list_head)->n, __typeof__(item), member),     \
	     back = LIST_ELEM(item->member.n, __typeof__(item), member);     \
	     &item->member != (list_head);                                \
	     item = back, back = LIST_ELEM(back->member.n, __typeof__(back), member))
#else
#define list_for_each_entry_safe(item, back, list_head, member)           \
	for (item = LIST_ELEM((list_head)->n, typeof(item), member),     \
